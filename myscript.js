...
function player() {
  if (noWinOrLose === 48) {
    ...
    $(".phedar")
      .text("Player One : Lord Voldemort")
      .css("color", "#D9AC2A");
    //startAgin()
  }
  if (a == 0) {
    ...
    $(".phedar")
      .text("Player Two : Albus Dumbledore")
      .css("color", "#D50B53");
  ...
  $("#gameBord").click(function() {
    ...
    for (var i = 0; i < arr.length; i++) {
      for (var j = 0; j < arr[i].length; j++) {
        //columns
        if (
          $(arr[i][j]).hasClass("yellow") &&
          $(arr[i][j + 1]).hasClass("yellow") &&
          $(arr[i][j + 2]).hasClass("yellow") &&
          $(arr[i][j + 3]).hasClass("yellow")
        ) {
          resultFun("Lord Voldemort won :(", ".resultP1");
        }

        if (
          $(arr[i][j]).hasClass("pink") &&
          $(arr[i][j + 1]).hasClass("pink") &&
          $(arr[i][j + 2]).hasClass("pink") &&
          $(arr[i][j + 3]).hasClass("pink")
        ) {
        ...
        if (i > arr.length - 4) {
          ...
          if (
            $(arr[i][j]).hasClass("yellow") &&
            $(arr[i - 1][j]).hasClass("yellow") &&
            $(arr[i - 2][j]).hasClass("yellow") &&
            $(arr[i - 3][j]).hasClass("yellow")
          ) {
            resultFun("Lord Voldemort won :(", ".resultP1");
          }
          ...
          if (
            $(arr[i][j]).hasClass("pink") &&
            $(arr[i - 1][j]).hasClass("pink") &&
            $(arr[i - 2][j]).hasClass("pink") &&
            $(arr[i - 3][j]).hasClass("pink")
          ) {
            resultFun("Albus Dumbledore won :)", ".resultP2");
          }
        }
        if (i <= arr.length - 4) {
          ...
          if (
            $(arr[i][j]).hasClass("yellow") &&
            $(arr[i + 1][j + 1]).hasClass("yellow") &&
            $(arr[i + 2][j + 2]).hasClass("yellow") &&
            $(arr[i + 3][j + 3]).hasClass("yellow")
          ) {
            resultFun("Lord Voldemort won :(", ".resultP1");
          }
          ...
          if (
            $(arr[i][j]).hasClass("pink") &&
            $(arr[i + 1][j + 1]).hasClass("pink") &&
            $(arr[i + 2][j + 2]).hasClass("pink") &&
            $(arr[i + 3][j + 3]).hasClass("pink")
          ) {
            resultFun("Albus Dumbledore won :)", ".resultP2");
          }
...
function resultFun(txt, pClass) {
  ...
  if (a == 1) {
    if (resultOne == 0) {
      ...
      $(
        ".firstCol , .secondCol , .thirdCol , .fourCol , .fiveCol , .sixCol , .sevenCol , .eightCol "
      ).off("click");
      $("<audio></audio>")
        .attr({
          src: "audio/win.mp3",
